...
def knapsack(luggage: list):
    ...
    luggage.sort(key=value)
    container_id = 1
    bag_id = 0
    while bag_id != len(luggage):
        bag = luggage[bag_id]
        if float(containers[container_id]['weight']) + float(bag['weight']) <= wt_container\
            and float(containers[container_id]['volume']) + bag_vol(bag) <= vol_container:
